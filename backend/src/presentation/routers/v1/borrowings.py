...
def create_new_borrowing(
    ...
    return borrowing_ops.create_borrowing(borrowing_repo, borrowing=borrowing)
...
def get_all_borrowings(
    ...
    status: Optional[borrowing_schema.BorrowStatus] = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    """
    ...
    borrowings = borrowing_ops.get_borrowings(borrowing_repo, skip=skip, limit=limit, user_id=query_user_id, status=status)
    return borrowings
...
def read_borrowing_record(
    ...
    # Authorization check
    if current_user.role != 'admin' and db_borrowing.user_id != current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def update_borrowing_record(
    ...
    """
    borrowing_repo = BorrowingRepositoryImpl(db)
    updated_borrowing = borrowing_ops.update_borrowing(borrowing_repo, borrow_id, borrowing)
...
def remove_borrowing_record(
    ...
    """
    borrowing_repo = BorrowingRepositoryImpl(db)
    success = borrowing_ops.delete_borrowing(borrowing_repo, borrow_id)
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Borrowing record not found")
