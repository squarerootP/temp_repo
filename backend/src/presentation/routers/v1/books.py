...
def get_all_books(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    get_book_use_case = book_ops.GetBookUseCase(book_repo=book_repo)
    books = get_book_use_case.get_books(skip=skip, limit=limit)
    return books


@router.get("/search", response_model=List[book_schema.BookResponse])
def search_books(
    text_to_search: str, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    """
    Retrieve all books with matching information, default to 0-100
    """
    book_repo = BookRepositoryImpl(db)
    search_book_use_case = book_ops.SearchBookUseCase(book_repo=book_repo)
    books = search_book_use_case.execute(text_to_search=text_to_search, skip=skip, limit=limit
    )
    return books
...
def read_book_by_isbn(isbn: str, db: Session = Depends(get_db)):
    ...
    try:
        get_book_use_case = book_ops.GetBookUseCase(book_repo=book_repo)
        db_book = get_book_use_case.get_book_by_isbn(isbn=isbn)
    except BookNotFound:
        raise HTTPException(
...
@router.post(
    "/", response_model=book_schema.BookResponse, status_code=status.HTTP_201_CREATED
)
def add_book(
    ...
    book_repo = BookRepositoryImpl(db)
    try:
        create_book_use_case = book_ops.CreateBookUseCase(book_repo=book_repo)
        return create_book_use_case.execute(current_user=current_user, book=Book(**book.model_dump()))
...
def update_book(
    ...
    book_repo = BookRepositoryImpl(db)
    try:
        update_book_use_case = book_ops.UpdateBookUseCase(book_repo=book_repo)
        updated_book = update_book_use_case.execute(current_user=current_user, book_isbn=isbn, book_data = book.model_dump(exclude_unset=True)
        )
    except BookNotFound:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Book not found"
...
def delete_book(
    ...
    book_repo = BookRepositoryImpl(db)
    try:
        del_book_use_case = book_ops.DeleteBookUseCase(book_repo=book_repo)
        del_book_use_case.execute(current_user=current_user, book_isbn=isbn)
    except BookNotFound:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Book not found"
        )
